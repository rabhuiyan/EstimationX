...
image_root_dir = './pm-iris-dataset/warsaw-nij-cropped-rgb-images/'
train_data_path = './train-test-data-list/dataset-disjoint/warsaw-cropped-RGB-image-metadata.txt'
test_data_path = './train-test-data-list/dataset-disjoint/nij-cropped-RGB-image-metadata.txt'
...
num_epochs = 500
best_mae = float('inf')
best_model_path = f"{output_dir}{model_name}_RGB_opt_{solver_name}_pret_{str(pretrained)}_wd_{str(weight_decay)}_best_model.pth"
...
plt.legend()
plt.grid(True)
plt.savefig(f"{output_dir}{model_name}_RGB_plot.pdf", format='pdf', dpi=3000)
plt.clf()
...
results.to_csv(f"{output_dir}{model_name}_RGB_test_result.txt", sep=',', index=False)
