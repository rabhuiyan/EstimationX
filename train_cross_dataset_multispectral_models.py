...
train_data_path = './train-test-data-list/dataset-disjoint/nij-cropped-multispectral-image-metadata.txt'
test_data_path = './train-test-data-list/dataset-disjoint/warsaw-cropped-multispectral-image-metadata.txt'
output_dir = './dataset-disjoint-result/test-on-warsaw/'
...
num_epochs = 500
best_mae = float('inf')
best_model_path = f"{output_dir}{model_name}_multispectral_opt_{solver_name}_pret_{str(pretrained)}_wd_{str(weight_decay)}_best_model.pth"
...
plt.legend()
plt.grid(True)
plt.savefig(f"{output_dir}{model_name}_multispectral_plot.pdf", format='pdf', dpi=3000)
plt.clf()
...
results.to_csv(f"{output_dir}{model_name}_multispectral_test_result.txt", sep=',', index=False)